...
This repo includes a `Dockerfile` and `render.yaml` for deploying the full
backend app on Render with PostgreSQL. The service must run as a long-lived web
process because it controls a Playwright browser during meetings.
...
DATABASE_URL
SLACK_WEBHOOK_URL
AUTH_STATE_JSON_BASE64
```

`AUTH_STATE_JSON_BASE64` should be the base64-encoded contents of
`auth-state.json` generated by `npm run auth-setup`. The app writes it to
`AUTH_STATE_PATH` at startup when the file is not already present.
